...
import { CloudUpload, PlusSquare, BarChart3, Settings, LogOut, GraduationCap } from 'lucide-react';
import Button from './Button';


const SideBar = ({ activeTab, onLogout }) => {
  const menuItems = [
    { id: 'upload', label: 'Upload Resources', icon: <CloudUpload size={20} /> },
    { id: 'quiz', label: 'Create Quiz', icon: <PlusSquare size={20} /> },
    { id: 'performance', label: 'Performance summary', icon: <BarChart3 size={20} /> },
  ];
        ...
        {menuItems.map((item) => (
          <div 
            ...
            className={`flex items-center gap-3 px-4 py-3 rounded-xl text-[13px] font-semibold cursor-pointer transition-all ${
              activeTab === item.id ? 'bg-[#E9F3F2] text-[#0D685E] border-r-4 border-[#0D685E]' : 'text-gray-600 hover:bg-gray-50'
            }`}
          >
